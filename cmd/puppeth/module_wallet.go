...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'net --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'net --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ethstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppethNetworkID/{{.NetworkID}}/g' dist/js/neterwallet-master.js && \
	sed -i 's/PuppethNetwork/{{.Network}}/g'     dist/js/neterwallet-master.js && \
	sed -i 's/PuppethDenom/{{.Denom}}/g'         dist/js/neterwallet-master.js && \
	sed -i 's/PuppethHost/{{.Host}}/g'           dist/js/neterwallet-master.js && \
	sed -i 's/PuppethRPCPort/{{.RPCPort}}/g'     dist/js/neterwallet-master.js
...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	rpcPort := infos.portmap["55555/tcp"]
