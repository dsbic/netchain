...
func (w *wizard) deployNode(boot bool) {
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{port: 44444, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{port: 44444, peersTotal: 50, peersLight: 0, gasTarget: 4.7, gasPrice: 18}
		}
	}
	existed := err == nil
	...
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an neterbase to mine against
			fmt.Println()
			if infos.neterbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.neterbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.neterbase)
				infos.neterbase = w.readDefaultAddress(common.HexToAddress(infos.neterbase)).Hex()
			}
