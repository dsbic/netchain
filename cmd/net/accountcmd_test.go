...
func TestAccountListEmpty(t *testing.T) {
	net := runNet(t, "account", "list")
	net.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	net := runNet(t, "account", "list", "--datadir", datadir)
	defer net.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	net := runNet(t, "account", "new", "--lightkdf")
	defer net.ExpectExit()
	net.Expect(`
...
`)
	net.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	net := runNet(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer net.ExpectExit()
	net.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(net.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(net.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	net.SetTemplateFunc("keypath", func(file string) string {
