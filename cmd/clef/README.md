...
The general flow for signing a transaction using e.g. net is as follows:
![image](sign_flow.png)

In this case, `net` would be started with `--externalsigner=http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
put together is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: Geth should be started in `net --external_signer localhost:8550`.
...
* Another potential thing to introduce is pairing.
  ...
  * Thus net/mist/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
