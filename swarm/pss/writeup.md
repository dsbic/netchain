...
This document aims to explain the changes in https://github.com/netersphere/go-blockchain/pull/126 and how those changes affect the pss_test.go TestNetwork tests.
...
Ultimately the deadlocks happen due to blocking `pp.Send()` call at:
 ...
 `p2p` request handling is synchronous (as discussed at https://github.com/netersphere/go-blockchain/issues/130), `pss` is also synchronous, therefore if two nodes happen to be processing a request, while at the same time waiting for response on `pp.Send(msg)`, deadlock occurs.
