...
#
# The cluster contains a bootnode, a net node and multiple swarm nodes, with
...
# static net configuration
GNET_IP="192.168.33.3"
GNET_RPC_PORT="55555"
GNET_RPC_URL="http://${GNET_IP}:${GNET_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the net and
# swarm nodes
start_bootnode() {
...
# start_net_node starts a net node with --datadir pointing at <base-dir>/net
# and a single, unlocked account with password "net"
start_net_node() {
  local dir="${base_dir}/net"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get net account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GNET_IP}"
    --rpcport   "${GNET_RPC_PORT}"
    --verbosity "6"
  )

  start_node "net" "${GNET_IP}" "$(which net)" ${args[@]}
}
...
create_account() {
  ...
  net --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
