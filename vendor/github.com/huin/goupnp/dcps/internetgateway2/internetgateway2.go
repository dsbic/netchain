...
const (
	...
	URN_WANNeternetLinkConfig_1    = "urn:schemas-upnp-org:service:WANNeternetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Neternet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Neternet
//
...
// WANNeternetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANNeternetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANNeternetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANNeternetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANNeternetLinkConfig1Clients() (clients []*WANNeternetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANNeternetLinkConfig_1); err != nil {
		return
	}
	clients = newWANNeternetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANNeternetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANNeternetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANNeternetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANNeternetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANNeternetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANNeternetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANNeternetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANNeternetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANNeternetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANNeternetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANNeternetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANNeternetLinkConfig1 {
	clients := make([]*WANNeternetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANNeternetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewNeternetLinkStatus: allowed values: Up, Down
func (client *WANNeternetLinkConfig1) GetNeternetLinkStatus() (NewNeternetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewNeternetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANNeternetLinkConfig_1, "GetNeternetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewNeternetLinkStatus, err = soap.UnmarshalString(response.NewNeternetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
